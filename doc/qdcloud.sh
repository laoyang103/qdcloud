...
systemctl restart tftpd && systemctl enable tftpd
...
cp doc/conf/named.jxit.net.cn /var/named/
...
echo "                                    用ansible开始部署计算集群                                             "
echo "=========================================================================================================="

# 将预先生成的公钥和私钥拷贝到管理节点
cp /root/laboratory/doc/conf/sshkey/idrsa* ~/.ssh/
chmod 600 ~/.ssh/idrsa
...
# 所有计算节点拷贝网卡和网桥配置样例文件，此处需要自己去每个计算节点配置桥接
# 配置好后用 systemctl restart network重启生效，远程操作有风险！
cd /root/laboratory/
